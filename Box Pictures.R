...
for(Picture in gsub("\\.png$", "", dir(), ignore.case = T)){
PNG <- readPNG(paste(Picture, ".png", sep = ""))[,,1:3]
...
CheckSlice <- function(Slice){
  ...
  MaxSumVal <- .25
  MaxDiffVal <- .3
  if(SliceRAbsAvg > MaxSolVal || SliceGAbsAvg > MaxSolVal || SliceBAbsAvg > MaxSolVal || SliceRAbsAvg+SliceGAbsAvg+SliceBAbsAvg > MaxSumVal || max(SliceRAbs) > MaxDiffVal || max(SliceGAbs) > MaxDiffVal || max(SliceBAbs) > MaxDiffVal){
